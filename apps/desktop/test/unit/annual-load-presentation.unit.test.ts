...
describe('annual load presentation', () => {
  it('builds a daily view and annual factor from the stored weather timestamps', () => {
    ...
    const project = { load: { activeProfileId: 'p', irMin: 10, calendar, profiles: [{ id: 'p', name: 'p', color: '#fff', source: 'hourly', appliances: [], hourly: Array.from({ length: 24 }, (_, hour) => ({ hour, realPower: hour < 12 ? 1 : 0, peakPower: 1 })), meter: null }] }, site: { timezoneIana: 'UTC', downloadedSource: { hourlyIrradiance: timestamps.map((timestamp) => ({ timestampUtcIso: timestamp, ghiWm2: 0, dniWm2: 0, dhiWm2: 0 })) } } } as unknown as ProjectViewModel;
