...
function MainCost({ role, quantity, total, unitPrice, margin, money, onPrice, onMargin }: { readonly role: string; readonly quantity: number; readonly total: number | null; readonly unitPrice: number; readonly margin: number; readonly money: string; readonly onPrice: (value: number) => void; readonly onMargin: (value: number) => void }) {
  return <div className="costrow"><span className="costrow-role">{role}</span><span className="costrow-qty"><b>{fmt(quantity)}</b><span className="unit">u</span></span><div className="costrow-fld"><NumField label="" unit={money} value={unitPrice} onChange={onPrice} /></div><div className="costrow-fld"><NumField label="" unit="%" value={margin} onChange={onMargin} decimals={1} /></div><span className="costrow-total"><b>{total === null ? '—' : fmt(total)}</b><span className="unit">{money}</span></span></div>;
}
...
export function SectionChiffrage() {
  ...
  // Le dossier porte sa devise ; l'écran ne peut pas en afficher une autre.
  ...
  return <div className="sheet">
    ...
    <section><div className="tbl-title"><h2 className="h-sec">{t('costing.mainEquipment')}</h2><span className="sep" /><span className="label">{t('costing.unitPrices')}</span></div><div className="costlist"><div className="costhead"><span /><span>{t('loads.quantity')}</span><span>{t('costing.unitCost')}</span><span>{t('costing.margin')}</span><span className="ta-r">{t('costing.saleTotal')}</span></div>
      <MainCost money={money} role="Modules" quantity={sizing?.pv.totalModules ?? 0} total={result?.lines.find((line) => line.key === 'modules')?.totalSale ?? null} unitPrice={effective.moduleUnitPrice} margin={effective.moduleMargin} onPrice={set('moduleUnitPrice')} onMargin={set('moduleMargin')} />
      <MainCost money={money} role="Batteries" quantity={sizing?.battery.totalUnits ?? 0} total={result?.lines.find((line) => line.key === 'batteries')?.totalSale ?? null} unitPrice={effective.batteryUnitPrice} margin={effective.batteryMargin} onPrice={set('batteryUnitPrice')} onMargin={set('batteryMargin')} />
      <MainCost money={money} role="Onduleurs" quantity={sizing?.inverter.count ?? 0} total={result?.lines.find((line) => line.key === 'inverters')?.totalSale ?? null} unitPrice={effective.inverterUnitPrice} margin={effective.inverterMargin} onPrice={set('inverterUnitPrice')} onMargin={set('inverterMargin')} />
    </div></section>
    <section><div className="tbl-title"><h2 className="h-sec">{t('costing.other')}</h2><span className="sep" /><span className="label">{costing.definedCostForAccessories ? 'en valeur' : 'en % du matériel principal'}</span><button className="btn" disabled={mainCost <= 0} onClick={toggleAccessoryMode}>{t('costing.toggle')}</button></div><div className="acclist">
      {ACCESSORIES.map((accessory) => { const priceKey = `${accessory.key}Price` as keyof typeof costing; const marginKey = `${accessory.key}Margin` as keyof typeof costing; return <div className="accitem" key={accessory.key}><span className="accitem-lbl">{accessory.label}</span><div className="accitem-pair"><div className="accrow-fld"><NumField label="" unit={costing.definedCostForAccessories ? money : '%'} value={costing[priceKey] as number} onChange={set(priceKey)} decimals={costing.definedCostForAccessories ? 0 : 1} /></div><div className="accrow-fld accrow-fld-m"><NumField label="" unit="%" value={costing[marginKey] as number} onChange={set(marginKey)} decimals={1} /></div></div></div>; })}
    </div></section>
    <section className={'out ' + (result ? '' : 'is-stale')}><div className="out-head"><span className="out-tag">{result ? 'calculé' : 'en attente'}</span><h2 className="h-sec">{t('costing.salePrice')}</h2><span className="sep" /><button className="btn" disabled={!result} onClick={() => setShowDetail(true)}>Détail par poste</button></div><div className="out-grid finance-summary">{[
      ['Total TTC', result?.totalTtc, money], ['Vente HT', result?.totalSaleHt, money], ['Bénéfice', result?.profit, money], ['Marge bénéficiaire', result === null ? undefined : result.averageMarginRatio * 100, '%'], ['Prix du Wc', result?.wattPeakPrice, `${money}/Wc`], ['LCOE actualisé', result?.lifecycle.lcoeActualized, `${money}/kWh`],
    ...
    {result && presizing && sizing && <section className="finance-comparison"><div className="out-head"><span className="out-tag">simulé</span><h2 className="h-sec">Comparaison du système retenu</h2><span className="sep" /><span className="badge ok">valeurs réelles recalculées</span></div><div className="tbl-wrap"><table className="tbl finance-compare-table"><thead><tr><th>Grandeur</th><th>Prédimensionné</th><th>Système retenu</th><th>Écart</th></tr></thead><tbody>{[
      ...
      ['LCOE actualisé', presizing.lcoe, result.lifecycle.lcoeActualized, `${money}/kWh`, true],
    ...
    {showLife && result && <Dialog title={'Cycle de vie sur ' + project.assumptions.projectLifetime + ' ans'} lead={'système réellement retenu · actualisé à ' + fmt(project.assumptions.actualizationRate, 1) + ' %'} wide onClose={() => setShowLife(false)}><div className="kpis" style={{ marginBottom: 0 }}><div className="kpi kpi-head"><span className="h-sec">Dépenses actualisées</span></div><div className="kpi"><span>Investissement initial TTC</span><span><b>{fmt(result.totalTtc)}</b><span className="unit">{money}</span></span></div><div className="kpi"><span>Entretien annuel</span><span><b>{fmt(result.lifecycle.annualMaintenanceCost)}</b><span className="unit">{money}/an</span></span></div><div className="kpi"><span>Remplacements actualisés</span><span><b>{fmt(result.lifecycle.actualizedReplacementCost)}</b><span className="unit">{money}</span></span></div><div className="kpi kpi-head"><span className="h-sec">Performance réelle</span></div><div className="kpi"><span>LCOE actualisé</span><span><b>{fmt(result.lifecycle.lcoeActualized)}</b><span className="unit">{money}/kWh</span>{presizing && <span className="delta">prédimensionné {fmt(presizing.lcoe)}</span>}</span></div><div className="kpi"><span>SVI · rapport au tarif réseau</span><span><b>{fmt(result.lifecycle.svi, 2)}</b>{presizing && <span className="delta">prédimensionné {fmt(presizing.svi, 2)}</span>}</span></div><div className="kpi"><span>Énergie servie annuelle</span><span><b>{fmt(result.simulation.servedEnergyKwh)}</b><span className="unit">kWh/an</span></span></div><div className="kpi kpi-head"><span className="h-sec">Impact évité</span></div><div className="kpi"><span>Groupe électrogène comparable</span><span><b>{fmt(result.lifecycle.dieselEquivalentInvestment)}</b><span className="unit">{money}</span><span className="delta">investissement seul, hors carburant et entretien</span></span></div><div className="kpi"><span>CO₂ évité</span><span><b>{fmt(result.lifecycle.co2AvoidedKg)}</b><span className="unit">kg</span></span></div><div className="kpi"><span>Équivalent arbres</span><span><b>{fmt(result.lifecycle.co2AvoidedTrees)}</b></span></div></div></Dialog>}
