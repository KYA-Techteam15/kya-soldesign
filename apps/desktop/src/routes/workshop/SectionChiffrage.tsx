...
export function SectionChiffrage() {
  ...
  return <div className="sheet">
    ...
    <section className="targets"><span className="targets-tag">{t('costing.quantities')}</span>{['Modules', 'Batteries', 'Onduleurs'].map((label) => <div className="tgt" key={label}><span className="tgt-lbl">{label}</span><span className="tgt-val"><b>—</b><span className="unit">u</span></span></div>)}</section>
    ...
    <section className="out is-stale"><div className="out-head"><span className="out-tag">{t('g.unavailable')}</span><h2 className="h-sec">{t('costing.salePrice')}</h2></div><div className="out-grid out-grid-4">{['Total TTC', 'Vente HT', `TVA ${costing.tvaPercent} %`, 'Prix du Wc'].map((label) => <div className="out-cell is-pending" key={label}><span className="out-lbl">{label}</span><span className="out-val"><b>—</b></span></div>)}</div></section>
