...
export function SectionMateriel() {
  ...
  return <div className="sheet">
    ...
    <section className="targets"><span className="targets-tag">à couvrir</span>{['Champ PV · kWc', 'Stockage · kWh', 'Onduleur · kW'].map((label) => <div className="tgt" key={label}><span className="tgt-lbl">{label}</span><span className="tgt-val"><b>—</b></span></div>)}</section>
    <CapabilityNotice capability="sizing" state={sizing} compact />
    <section><h2 className="h-sec">Composants au catalogue</h2><div className="picklist">
    ...
    <section><div className="tbl-title"><h2 className="h-sec">Compatibilité des équipements</h2><span className="sep" /></div><CapabilityNotice capability="equipment-compatibility" state={compatibility} /></section>
    <div className="runbar is-stale"><button className="btn btn-ok btn-run" disabled>Dimensionnement indisponible</button><span className="runbar-note">Les références sont enregistrées sans recommandation, quantité ni réserve fabriquée.</span></div>
    <section className="out is-stale"><div className="out-head"><span className="out-tag">indisponible</span><h2 className="h-sec">Système retenu</h2></div><div className="fitgrid">{['Champ PV', 'Parc batteries', 'Onduleurs'].map((label) => <div className="fitcell" key={label}><span className="out-lbl">{label}</span><div className="fitline"><span>Requis</span><b>—</b></div><div className="fitline"><span>Obtenu</span><b>—</b></div></div>)}</div></section>
