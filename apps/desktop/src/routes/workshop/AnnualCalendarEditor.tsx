...
const MODE_LABELS: Record<LoadCalendarView['mode'], string> = {
  annual: 'calendar2.profilUniquePourToute',
  'workweek-weekend': 'calendar2.ouvresEtWeekEnd',
  periods: 'calendar2.periodes',
  'periods-by-day-type': 'calendar2.periodesTypesDeJour',
};
...
export function calendarForMode(mode: LoadCalendarView['mode'], profileId: string): LoadCalendarView {
  ...
  const periods = [{ id: 'annual', name: tr('calendar2.year'), startMonthDay: '01-01', endMonthDay: '12-31' }];
  return {
...
export function AnnualCalendarEditor({ calendar, profiles, activeProfileId, onChange, onActivateProfile, onAddProfile }: {
  ...
  const addPeriod = () => {
    ...
    const period = { id, name: fill(t('calendar2.periodN'), { n: calendar.periods.length + 1 }), startMonthDay: '01-01', endMonthDay: '12-31' };
  ...
  const modeLabel = calendar.mode === 'annual' ? t('loads.calendarAnnualTitle') : t(MODE_LABELS[calendar.mode]);
  return <section className="calendar-editor" aria-label={t('loads.calendarTitle')}>
    ...
    <div className="calendar-matrix">
      ...
      {calendar.periods.map((period) => <div className="calendar-matrix-row" key={period.id}>
        <div className="calendar-period"><input aria-label={fill(t('calendar2.periodName'), { name: period.name })} value={period.name} onChange={(event) => setPeriod(period.id, 'name', event.target.value)} />{calendar.mode === 'periods-by-day-type' && <div><input aria-label={`${t('calendar2.startOf')} ${period.name}`} placeholder="MM-JJ" value={period.startMonthDay} onChange={(event) => setPeriod(period.id, 'startMonthDay', event.target.value)} /><span>→</span><input aria-label={`${t('calendar2.endOf')} ${period.name}`} placeholder="MM-JJ" value={period.endMonthDay} onChange={(event) => setPeriod(period.id, 'endMonthDay', event.target.value)} /></div>}</div>
