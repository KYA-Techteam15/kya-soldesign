...
export function SectionDossier() {
  ...
  return <div className="sheet"><StepHead slug="dossier" aside={<div className="seg">{TABS.map((item) => <button key={item.key} aria-selected={tab === item.key} onClick={() => setParams(item.key === 'synthese' ? {} : { vue: item.key }, { replace: true })}>{item.label}</button>)}</div>} />
    ...
    <section className="out is-stale"><div className="out-head"><span className="out-tag">{t('g.unavailable')}</span><h2 className="h-sec">{tab === 'synthese' ? 'Synthèse du dossier' : tab === 'synoptique' ? 'Synoptique de l’installation' : 'Documents client'}</h2></div><div className="out-grid">{['Projet et client', 'Site et climat', 'Besoins', 'Système retenu', 'Fiabilité', 'Chiffrage'].map((label) => <div className="out-cell is-pending" key={label}><span className="out-lbl">{label}</span><span className="out-val"><b>—</b></span></div>)}</div></section>
  </div>;
}
