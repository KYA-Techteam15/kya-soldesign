...
export function SectionSite() {
  ...
  return <div className="sheet">
    ...
    <section><div className="tbl-title"><h2 className="h-sec">{t('site.weatherSource')}</h2><span className="label">{source ? `${source.sourceName} · ${source.provider}` : site.localityId ? 'aucune source liée à cette localité' : 'aucune localité choisie'}</span></div></section>
    <div className="chart-duo">
      <section><div className="tbl-title"><h2 className="h-sec">{t('site.monthlyIrradiation')}</h2><span className="label">kWh/m²/jour en plan des modules</span></div><div className="tbl-wrap" style={{ padding: 'var(--sp-3)' }}><div className="empty" style={{ margin: 0 }}><b>{t('site.noWeatherSeries')}</b>Les métadonnées canoniques identifient la source, mais ne contiennent pas de valeurs horaires. Aucun profil n’est fabriqué.</div></div></section>
      <section><div className="tbl-title"><h2 className="h-sec">{t('site.averageDay')}</h2><span className="label">W/m² en plan des modules</span></div><div className="tbl-wrap" style={{ padding: 'var(--sp-3)' }}><div className="empty" style={{ margin: 0 }}><b>{t('site.noTypicalDay')}</b>Elle sera produite à partir d’une vraie série météo par une capacité dédiée.</div></div></section>
    </div>
