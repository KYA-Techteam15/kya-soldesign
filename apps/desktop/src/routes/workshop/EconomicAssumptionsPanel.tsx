...
export function EconomicAssumptionsPanel({ assumptions, onSet, onSetMode, onApplyReference }: {
  ...
  return <div className="economic-assumptions">
    <CostGroup title={t('presizing.costs.pv')} mode={assumptions.pvCostInputMode} specificCost={assumptions.pvSpecificCost} margin={assumptions.pvMargin} referencePrice={assumptions.pvReferencePrice} referenceSize={assumptions.pvReferencePowerW} sizeLabel={t('presizing.costs.panelPower')} sizeUnit="Wc" divisor={1000} specificUnit="FCFA/kWc" onMode={(mode) => onSetMode('pv', mode)} onSpecific={(value) => onSet('pvSpecificCost', value)} onMargin={(value) => onSet('pvMargin', value)} onApply={(price, size, cost) => onApplyReference('pv', price, size, cost)} />
    <CostGroup title={t('presizing.costs.storage')} mode={assumptions.storageCostInputMode} specificCost={assumptions.batterySpecificCost} margin={assumptions.batteryMargin} referencePrice={assumptions.storageReferencePrice} referenceSize={assumptions.storageReferenceKwh} sizeLabel={t('presizing.costs.referenceStorage')} sizeUnit="kWh" divisor={1} specificUnit="FCFA/kWh" onMode={(mode) => onSetMode('storage', mode)} onSpecific={(value) => onSet('batterySpecificCost', value)} onMargin={(value) => onSet('batteryMargin', value)} onApply={(price, size, cost) => onApplyReference('storage', price, size, cost)} />
    <CostGroup title={t('presizing.costs.inverter')} mode={assumptions.inverterCostInputMode} specificCost={assumptions.inverterSpecificCost} margin={assumptions.inverterMargin} referencePrice={assumptions.inverterReferencePrice} referenceSize={assumptions.inverterReferencePowerW} sizeLabel={t('presizing.costs.inverterPower')} sizeUnit="W" divisor={1000} specificUnit="FCFA/kW" onMode={(mode) => onSetMode('inverter', mode)} onSpecific={(value) => onSet('inverterSpecificCost', value)} onMargin={(value) => onSet('inverterMargin', value)} onApply={(price, size, cost) => onApplyReference('inverter', price, size, cost)} />
    <EconomicGroup title={t('presizing.costs.environment')} method={t('presizing.costs.environmentMethod')}><div className="form-rows economic-fields"><NumField label={t('presizing.costs.gridTariff')} unit="FCFA/kWh" value={assumptions.lcoeGrid} onChange={(value) => onSet('lcoeGrid', value)} /><NumField label={t('presizing.costs.emissionFactor')} unit="kgCO₂/kWh" value={assumptions.emissionFactor} onChange={(value) => onSet('emissionFactor', value)} decimals={2} /></div></EconomicGroup>
  </div>;
}

function CostGroup({ title, mode, specificCost, margin, referencePrice, referenceSize, sizeLabel, sizeUnit, divisor, specificUnit, onMode, onSpecific, onMargin, onApply }: {
  readonly title: string; readonly mode: CostMode; readonly specificCost: number; readonly margin: number; readonly referencePrice: number | null; readonly referenceSize: number | null; readonly sizeLabel: string; readonly sizeUnit: string; readonly divisor: number; readonly specificUnit: string; readonly onMode: (mode: CostMode) => void; readonly onSpecific: (value: number) => void; readonly onMargin: (value: number) => void; readonly onApply: (price: number, size: number, cost: number) => void;
}) {
  const t = useT();
  ...
  return <EconomicGroup title={title} method={mode === 'specific' ? t('presizing.costs.specificMethod') : t('presizing.costs.componentMethod')}>
    ...
    {mode === 'specific' ? <div className="form-rows economic-fields"><NumField label={t('presizing.costs.specificCost')} unit={specificUnit} value={specificCost} onChange={onSpecific} /><NumField label={t('presizing.costs.commercialMargin')} unit="%" value={margin} onChange={onMargin} decimals={1} /></div> : <><div className="form-rows economic-fields"><NumField label={sizeLabel} unit={sizeUnit} value={size} onChange={setSize} decimals={sizeUnit === 'kWh' ? 2 : 0} /><NumField label={t('presizing.costs.componentPrice')} unit="FCFA" value={price} onChange={setPrice} /><NumField label={t('presizing.costs.commercialMargin')} unit="%" value={margin} onChange={onMargin} decimals={1} /></div><div className="economic-conversion" role="status"><span>{t('presizing.costs.calculatedSpecific')}</span><b>{preview === null ? '—' : fmt(preview)} <span className="unit">{specificUnit}</span></b><small>{preview === null ? t('presizing.costs.referenceRequired') : `${fmt(price)} ÷ ${fmt(size / divisor, divisor === 1 ? 2 : 3)} = ${fmt(preview)}`}</small><button type="button" className="btn btn-ok" disabled={preview === null} onClick={() => { if (preview !== null) onApply(price, size, preview); }}>{t('presizing.costs.applyCalculated')}</button></div></>}
