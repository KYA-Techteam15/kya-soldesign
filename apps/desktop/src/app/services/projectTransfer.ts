import { safeFileName } from '../platform/files.js';
...
export class ProjectTransferService {
  ...
  public async inspectImport(): Promise<{ readonly fileName: string; readonly inspection: ImportInspection } | null> { const file = await this.files.pickTextFile({ accept: '.ksd,.json,application/json' }); return file ? { fileName: file.name, inspection: inspectProjectImport(file.text, this.projects.list()) } : null; }
...
function safeFilename(project: ProjectFileV1): string { return `${safeFileName(project.name.trim() || project.id, 'projet')}.ksd`; }
