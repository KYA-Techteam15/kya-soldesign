...
export function buildAnnualLoadPresentation(project: ProjectViewModel, solar: SolarResourceAnalysisOutputV1 | null): AnnualLoadPresentation | null {
  ...
  if (activeProfile?.source === 'meter') return null;
  const annualProfiles = project.load.profiles.map((profile) => {
  ...
  try {
    const series = buildAnnualLoadSeries({
      timezoneIana: project.site.timezoneIana ?? 'UTC', weather, calendar: project.load.calendar,
    ...
    return { series, yEn, daily: [...byDate.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([dateIso, values]) => ({ dateIso, ...values })) };
  } catch {
    return null;
  }
}
