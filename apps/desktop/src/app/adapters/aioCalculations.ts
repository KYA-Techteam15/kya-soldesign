import type { AioSizingEnvelopeV1, AioSizingOutputV1 } from '@ksd/engine';
...
import { projectToAioInput } from './projectToAio.js';
...
export class AioCalculations implements CalculationCapabilityPort {
  ...
  public async read<Output>(projectId: string, capability: CapabilityId): Promise<CapabilityState<Output>> {
    if (capability !== 'presizing' && capability !== 'sizing') return unavailableCalculations.read(projectId, capability);
