...
describe('Page 1 load normalization', () => {
  ...
  it('normalizes equipment useful power, efficiency and schedules without losing energy', () => {
    const result = normalizeEquipmentRows({
      timezoneIana: 'Africa/Lome',
      rows: [{ id: 'pump', label: 'Pompe', quantity: 2, usefulPowerW: 500, efficiencyRatio: 0.8, hourlyOperatingFractions: Array.from({ length: 24 }, (_, hour) => hour >= 8 && hour < 12 ? 1 : 0), startupPowerMultiplier: 3 }],
    });
    expect(result.status).toBe('ready');
    if (result.status !== 'ready') return;
    expect(result.load.hourlyEnergyWh.reduce((sum, value) => sum + value, 0)).toBe(5000);
    expect(result.load.startupEvents).toEqual([{ hourIndex: 8, runningPowerW: 1250, startupPowerMultiplier: 3, isInductive: true, sourceRef: 'pump' }]);
  });
  ...
  it('treats a block crossing midnight as one start and a continuous load as one representative start', () => {
    ...
    for (const [schedule, expectedHour] of [[crossing, 22], [continuous, 0]] as const) {
      const result = normalizeEquipmentRows({ timezoneIana: 'Africa/Lome', rows: [{ id: 'motor', label: 'Moteur', quantity: 1, usefulPowerW: 100, efficiencyRatio: 1, hourlyOperatingFractions: schedule, startupPowerMultiplier: 2 }] });
      expect(result.status).toBe('ready');
  ...
  it('blocks incomplete equipment ratios instead of inventing defaults', () => {
    const result = normalizeEquipmentRows({ timezoneIana: 'Africa/Lome', rows: [{ id: 'load', label: 'Charge', quantity: 1, usefulPowerW: 100, efficiencyRatio: null, hourlyOperatingFractions: Array.from({ length: 24 }, () => 0), startupPowerMultiplier: null }] });
    expect(result).toMatchObject({ status: 'blocked', issues: [{ code: 'LOAD_EFFICIENCY_MISSING' }] });
  });
