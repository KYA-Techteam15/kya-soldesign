...
- [x] T061 (fait le 2026-09-26, branche `feat/012-t061-plateforme` ; reste à viser la production quand la plateforme l'ouvrira) **Bloquant avant vente réelle** : brancher `HttpAdminApi` sur la plateforme, remplacer `LICENSE_PUBLIC_KEY` par la clé publique de la plateforme, retirer `SimulatedAdminApi`, `DEMO_SIGNING_KEY`, `DEMO_KEYS` et la licence de démonstration automatique (la clé privée de démonstration est publique : tant qu'elle signe, n'importe qui peut forger une licence)
