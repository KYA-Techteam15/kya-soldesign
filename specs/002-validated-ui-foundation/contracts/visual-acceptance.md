...
## Recovery review ledger — 2026-08-13

| ID | Disposition | Evidence |
|---|---|---|
| VD-001 | Applied as approved | In-app source/target review plus `visual.spec.ts-snapshots/home-fr-1440-chromium-win32.png`; Home preserves the source composition and exposes only the empty in-memory session. |
...
| VD-004 | Applied as approved | Validated `tokens.css`, `validated.css`, and `print.css` are ported; only sober light/dark modes remain; Impeccable detector returned `[]`. |
...
**Acceptance result**: recovered and accepted on 2026-08-13. The live source at revision `1be343f...` and Next were compared directly in the in-app browser for Home, top-level routes, the eight-step workshop, palette/dialog behavior, and constrained desktop. The 36-snapshot FR/EN matrix is green at both viewports with a 250-pixel raster tolerance limited to dynamic draft IDs, Impeccable reports `[]`, and prototype numeric content remains intentionally excluded under VD-003.
